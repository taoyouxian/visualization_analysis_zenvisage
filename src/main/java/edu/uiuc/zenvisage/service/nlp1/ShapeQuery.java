...
public class ShapeQuery {
	...
	public ShapeQuery(List<ShapeSegment> shapeQuery){
		...
		for(ShapeSegment location : shapeQuery){
			this.shapeSegment.add(new ShapeSegment(location.modifier,location.keyword,location.x_start,location.x_end,location.y_start,location.y_end,location.operator,location.subSegments));
		}
	}
}
